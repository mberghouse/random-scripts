%interpstreamspeed_2d
function vertsout = interpstreamspeed2(varargin)
...
if any(any(isnan(cat(1, verts{:}))))
  error(message('MATLAB:interpstreamspeed:NaNVertices'))
end

if any(isnan(u(:))) ||  any(isnan(v(:))) ||  any(isnan(w(:))) 
  error(message('MATLAB:interpstreamspeed:NaNUVW'))
end
...
for i = 1:length(verts)
  ...
  if size(vv,1)==1
    ...
    while(pos<length(dis))
      ...
      if (length(vertsout{i})>50000) %This is the maximum length of trajectories (in frames)
          break
      end
      if s<1e-9
          break
...
if (nin==5 || nin==6) && ismatrix(vargin{3}) && iscell(vargin{5}) 
...
elseif (nin==3 || nin==4) && ismatrix(vargin{1})  && iscell(vargin{3}) 
  u = vargin{1}; % 2D interpstreamspeed(u,v,verts) or interpstreamspeed(u,v,verts,sf)
  v = vargin{2};
  verts = vargin{3};
  if nin==4, sf = vargin{4}; end
elseif (nin==4 || nin==5) && ismatrix(vargin{3}) && iscell(vargin{4}) 
...
elseif nin==2 || nin==3
  ...
  error(message('MATLAB:interpstreamspeed:WrongNumberOfInputs')); 
end